...
pub fn generate_random_bytes(length: u32) -> Result<Vec<u8>, Error> {
    let mut rng = OsRng::new().map_err(|_| ErrorKind::OsRng)?;
...
/// function is doing.
pub fn generate_random_base64_encoded_string_config(length: u32, config: base64::Config) -> Result<String, Error> {
    let mut rng = OsRng::new().map_err(|_| ErrorKind::OsRng)?;
    let mut bytes = vec![0u8; length as usize];
    rng.fill_bytes(&mut bytes);
